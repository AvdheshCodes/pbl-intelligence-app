...
export default function GrantReport() {
  ...
  // Update available months when grant changes
  useEffect(() => {
  ...
  // Load fact panel when grant + month are selected
  useEffect(() => {
    ...
    <>
        ...
        <div className="page-header-row">
          <div>
            <h1 className="page-title">
              Grant <span>Reporting Assistant</span>
            </h1>
      ...
      <div className="grant-selector-bar" role="search" aria-label="Grant selection">
        <div className="filter-group">
          <label className="filter-label" htmlFor="grant-select">Grant</label>
        ...
        <div className="filter-group">
          <label className="filter-label" htmlFor="month-select">Reporting Month</label>
        ...
        {f && (
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.6, color: 'var(--text-muted)', marginBottom: 4 }}>Report Status</div>
              <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)' }}>{f.reportStatus}</div>
            </div>
            <div className="filter-divider" style={{ margin: '0 8px' }} />
            <div>
              <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.6, color: 'var(--text-muted)', marginBottom: 4 }}>Program Risk</div>
      ...
      {f && (
        <div style={{ animation: 'page-fade-in var(--t-slow) var(--ease)' }}>
          <div className="fact-panel">
            ...
            <div className="card" role="region" aria-label="Performance Metrics">
              <div className="section-title" style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                <span style={{ padding: '6px', background: 'var(--indigo-dim)', borderRadius: 'var(--radius-sm)', color: 'var(--indigo-light)', lineHeight: 1, fontSize: 16 }}>📊</span>
                Performance Metrics
              </div>
              ...
              {[
                ...
                { k: 'Completed PBL',       v: `${fmt(f.schoolsCompletedPbl)} (${pct(f.pblCompletionRate)})` },
                { k: 'Evidence Submitted',  v: `${fmt(f.schoolsWithEvidence)} (${pct(f.evidenceSubmissionRate)})` },
              ...
              {f.trend && (
                <div style={{ marginTop: 16, padding: '12px 16px', background: 'var(--bg-input)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 24 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                      <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: 0.5, color: 'var(--text-muted)' }}>Completion</span>
                      <span style={{ fontSize: 13, fontWeight: 700, color: f.trend.completionDelta >= 0 ? 'var(--emerald)' : 'var(--rose)' }}>
                        {f.trend.completionDelta >= 0 ? '↑' : '↓'} {pct(Math.abs(f.trend.completionDelta))}
                      </span>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                      <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: 0.5, color: 'var(--text-muted)' }}>Attendance</span>
                      <span style={{ fontSize: 13, fontWeight: 700, color: f.trend.attendanceDelta >= 0 ? 'var(--emerald)' : 'var(--rose)' }}>
            ...
            <div className="card" role="region" aria-label="Budget and Milestones">
              <div className="section-title" style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                <span style={{ padding: '6px', background: 'var(--teal-dim)', borderRadius: 'var(--radius-sm)', color: 'var(--teal)', lineHeight: 1, fontSize: 16 }}>💰</span>
                Budget Utilization
              </div>
              ...
              {f.financeLines?.length > 0 ? (
                ...
                <div className="empty-state" style={{ padding: '24px 16px', marginBottom: 24 }}>
                  <span className="empty-icon" style={{ fontSize: 24 }}>💸</span>
                  <div style={{ fontSize: 12 }}>No finance data for this period</div>
                </div>
              )}
              ...
              {f.milestoneSummary && (
                <>
                  <div className="section-title" style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, marginTop: 12 }}>
                    <span style={{ padding: '6px', background: 'var(--amber-dim)', borderRadius: 'var(--radius-sm)', color: 'var(--amber)', lineHeight: 1, fontSize: 16 }}>📌</span>
                    Key Milestones
                  </div>
                  <ul className="milestone-list">
                    {f.milestoneSummary.split('|').map((m, i) => (
                      <li key={i} className="milestone-item">
                        <span className="milestone-dot" style={{ background: m.includes('Completed') ? 'var(--emerald)' : m.includes('Risk') ? 'var(--rose)' : 'var(--amber)' }} />
                        <span>{m.trim()}</span>
                      </li>
                    ))}
                  </ul>
          ...
          {f.evidenceRefs?.length > 0 && (
            <div style={{ marginBottom: 32 }} role="region" aria-label="Evidence Gallery">
              <div className="section-header">
                <div className="section-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ padding: '6px', background: 'var(--violet-dim)', borderRadius: 'var(--radius-sm)', color: 'var(--violet)', lineHeight: 1, fontSize: 16 }}>🖼</span>
                  Evidence & Media
                </div>
                <span className="section-badge" style={{ background: 'var(--violet-dim)', color: 'var(--violet)', borderColor: 'rgba(139,92,246,0.15)' }}>
                ...
                {f.evidenceRefs.map((ev) => (
                  <div key={ev.recordId} className="evidence-card">
                    ...
                    <div className="evidence-body">
                      ...
                      {ev.district && <div className="evidence-meta">📍 {ev.district}</div>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div style={{ marginBottom: 24, display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
            <button id="btn-generate-grant-report" className="btn btn-primary" onClick={handleGenerate} disabled={genLoading}>
              {genLoading ? <><span className="spinner" /> Generating…</> : '✨ Generate Report Section'}
            </button>
            {genResult && !genResult.aiEnabled && (
              <div className="ai-disabled-note">
                ⚠️ AI narrative generation is disabled by configuration.
              </div>
            )}
          </div>
            ...
            <div>
              {genResult.narrative ? (
                <div className="narrative-panel" role="region" aria-label="Generated Narrative">
                  ...
                  <p className="narrative-text">{genResult.narrative}</p>
                  <div className="narrative-footer">
                    ✓ Every sentence above is sourced from the structured facts above via deterministic templates.
                  </div>
                </div>
              ) : (
                <div className="narrative-panel" style={{ background: 'rgba(245,158,11,0.05)', borderColor: 'rgba(245,158,11,0.2)' }}>
                  <div className="narrative-label" style={{ color: 'var(--amber)' }}>📊 Facts Only Mode</div>
